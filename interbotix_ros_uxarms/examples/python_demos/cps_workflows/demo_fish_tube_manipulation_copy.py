...
host = 'localhost'
port = '1337'
abstract = '/home/viperx/Desktop/CPS-Project/niraapad/niraapad/sarwat/abstract_config_file.json'
...
def move_object_cart(start_positions, end_positions, object):
    viperx.gripper.open(delay=2)
...
if __name__ == '__main__':
    ...
    viperx = InterbotixManipulatorXS("vx300s", "arm", "gripper")
    #viperx.arm.go_to_sleep_pose()
    viperx.arm.set_ee_pose_components(x=0.536494,y=0,z=0.42705)
    
  

    # Start demo
    dosing_device.set_door("state", "open")
    move_object_cart(grid_nw, grid_sw, "vial")
    viperx.arm.set_ee_pose_components(x=0.536494,y=0,z=0.42705)
    dosing_device.set_door("state", "closed")
    dosing_device.run_action(delay=3, quantity=4)
    dosing_device.stop_action(delay=0)
    dosing_device.set_door("state", "open")
    move_object_cart(grid_sw, grid_ne, "vial")
    dosing_device.set_door("state", "closed")
    viperx.arm.set_ee_pose_components(x=0.536494,y=0,z=0.42705)
    start_shaking_soln(200, 2)
    ##move_object_cart(start_coords, goal_coords, "ring")
    stop_shaking_soln()
    move_object_cart(grid_ne, grid_nw, "vial")
    viperx.arm.set_ee_pose_components(x=0.536494,y=0,z=0.42705)
    viperx.arm.go_to_sleep_pose()
